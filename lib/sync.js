...
const supportsReturning = (config) => {
  let client = config.client
  console.log('the config is: ', config.returning)
  if(config.returning){
    return _.includes(['postgresql', 'postgres', 'pg', 'oracle', 'mssql'], client)
  }else{
    return false
  }
}
...
_.extend(Sync.prototype, {
  ...
  insert: Promise.method(function() {
    ...
    if(supportsReturning(this.query.client.config)){
      return this.query.insert(
        syncing.format(_.extend(Object.create(null), syncing.attributes)),
        supportsReturning(this.query.client.config) ? syncing.idAttribute : null
      );
    }else{
      ...
      console.log('the alt insert is' , insert)

      const new_insert = 'DECLARE @OutputTbl TABLE (id UNIQUEIDENTIFIER) ' + insert.replace('inserted.['+syncing.idAttribute+']', 'inserted.['+syncing.idAttribute+'] INTO @OutputTbl') + ' SELECT ['+syncing.idAttribute+'] FROM @OutputTbl'


      console.log('the new insert is' , new_insert)

      console.log('the query knex is: ', this.rawknex)
      console.log('the query obj is: ', this.query)

      return this.rawknex.raw(new_insert)

      // console.log('alt insert')
      // const secondQuery = this.query.clone()
      // const attributes = syncing.format(_.extend(Object.create(null), syncing.attributes))
      // const entries = Object.entries(attributes)
      // for (const [key, value] of entries) {

      //   var datetime = Date.parse(value)
      //   if(!Number.isNaN(datetime) && (typeof value != 'number') && (typeof value != 'string')){
      //     console.log('is date creating new value for ', value)
      //     var d = new Date();
